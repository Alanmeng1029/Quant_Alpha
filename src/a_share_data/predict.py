...
def backtest_targets(catalog: Path, target_weights: Path, output: Path, buy_bps: float = 2.0, sell_bps: float = 2.0, initial_capital: float = 10_000_000.0, lot_size: int = 100, return_basis: str = "qfq", rebalance_band: float = 0.0, benchmark_index_code: str = "000905.SH") -> dict:
    ...
    try:
        ...
        benchmark = pl.from_arrow(conn.execute("SELECT trade_date, open FROM index_daily WHERE index_code=? AND trade_date BETWEEN ? AND ? AND open > 0", [benchmark_index_code, execution_dates[0], execution_dates[-1]]).arrow()).with_columns(pl.col("trade_date").cast(pl.Date))
    finally: conn.close()
    ...
    for i, day in enumerate(execution_dates[:-1]):
        ...
        rows.append({"execution_date":day,"next_execution_date":next_day,"gross_return":gross,"transaction_cost":transaction_cost / previous_ending_value,"net_return":net,"benchmark_return":bench,"active_return":net-bench,"buy_turnover":bought / equity,"sell_turnover":sold / equity,"nav":nav,"benchmark_nav":bench_nav,"holding_count":len(shares),"cash":cash,"cash_weight":cash / equity,"equity":ending_value})
...
def render_backtest_report(portfolio_daily: Path, output: Path, title: str = "Portfolio backtest", target_weights: Path | None = None, benchmark_name: str = "CSI500") -> dict:
    ...
    benchmark_column = "benchmark_return" if "benchmark_return" in frame.columns else "csi500_return"
    required={"gross_return","net_return","transaction_cost","buy_turnover","sell_turnover",benchmark_column}
    missing=required-set(frame.columns)
    if missing: raise ValueError(f"portfolio daily file missing: {sorted(missing)}")
    gross=frame["gross_return"].to_numpy(); net=frame["net_return"].to_numpy(); csi=frame[benchmark_column].to_numpy(); cost=frame["transaction_cost"].to_numpy()
    ...
    metrics={"days":len(frame),"start":str(frame["execution_date"][0]),"end":str(frame["execution_date"][-1]),"benchmark":benchmark_name,"gross_total_return":float(gross_nav[-1]-1),"net_total_return":float(net_nav[-1]-1),"benchmark_total_return":float(csi_nav[-1]-1),"net_excess_wealth_vs_benchmark":net_excess_wealth,"gross_annualized_return":ann(gross),"net_annualized_return":net_ann,"benchmark_annualized_return":ann(csi),"net_annualized_excess_vs_benchmark":float((net_nav[-1] / csi_nav[-1]) ** (1 / years) - 1) if years else 0.0,"excess_curve_total_return":float(excess_curve[-1]-1.0),"excess_annual_return_243":excess_annual_return,"excess_sharpe_243":excess_sharpe,"excess_max_drawdown":excess_max_drawdown,"excess_calmar":excess_calmar,"composite_score":composite_score,"average_daily_active_return_bps":float(np.mean(active_returns)*10_000),"annualized_tracking_error":vol(active_returns),"net_sharpe":net_ann/vol(net) if vol(net) else None,"information_ratio":float(np.mean(active_returns)/np.std(active_returns,ddof=1)*np.sqrt(252)) if np.std(active_returns,ddof=1) else None,"max_drawdown":float(drawdown.min()),"average_buy_turnover":float(frame["buy_turnover"].mean()),"average_sell_turnover":float(frame["sell_turnover"].mean()),"average_fee_bps":float(np.mean(cost)*10_000),"cumulative_fee_paid_on_initial_nav":float(fee[-1]),"average_holding_count":float(frame["holding_count"].mean())}
...
def main(argv: list[str] | None = None) -> None:
    ...
    bt=sub.add_parser("backtest-portfolio"); bt.add_argument("--catalog",type=Path,required=True); bt.add_argument("--target-weights",type=Path,required=True); bt.add_argument("--output",type=Path,required=True); bt.add_argument("--buy-bps",type=float,default=2.0); bt.add_argument("--sell-bps",type=float,default=2.0); bt.add_argument("--initial-capital",type=float,default=10_000_000); bt.add_argument("--lot-size",type=int,default=100); bt.add_argument("--return-basis",choices=["qfq","raw"],default="qfq"); bt.add_argument("--benchmark-index-code",default="000905.SH")
    ...
    bp.add_argument("--daily-buy-budget",type=float,default=.10); bp.add_argument("--daily-sell-budget",type=float,default=.10); bp.add_argument("--h1-weight",type=float,default=.5); bp.add_argument("--entry-sizing",choices=["equal","rank_tilt"],default="equal"); bp.add_argument("--rank-tilt",type=float,default=.15); bp.add_argument("--initial-capital",type=float,default=10_000_000); bp.add_argument("--lot-size",type=int,default=100); bp.add_argument("--buy-bps",type=float,default=2.0); bp.add_argument("--sell-bps",type=float,default=2.0); bp.add_argument("--no-report",action="store_true")
    rp=sub.add_parser("render-backtest-report"); rp.add_argument("--portfolio-daily",type=Path,required=True); rp.add_argument("--output",type=Path,required=True); rp.add_argument("--title",default="Portfolio backtest"); rp.add_argument("--target-weights",type=Path); rp.add_argument("--benchmark-name",default="CSI500")
    ...
    so=sub.add_parser("sequence-oos", help="Rust-cache/PyTorch-MPS rolling LSTM comparison")
    so.add_argument("--config",type=Path,required=True); so.add_argument("--pilot",action="store_true"); so.add_argument("--max-windows",type=int); so.add_argument("--prepare-only",action="store_true"); so.add_argument("--skip-backtests",action="store_true")
    ...
    elif args.command=="backtest-portfolio": result=backtest_targets(args.catalog,args.target_weights,args.output,args.buy_bps,args.sell_bps,args.initial_capital,args.lot_size,args.return_basis,benchmark_index_code=args.benchmark_index_code)
    ...
    elif args.command=="sequence-oos":
        from a_share_data.sequence_oos import run_sequence_oos
        result=run_sequence_oos(args.config,pilot=args.pilot,max_windows=args.max_windows,prepare_only=args.prepare_only,skip_backtests=args.skip_backtests)
    else: result=render_backtest_report(args.portfolio_daily,args.output,args.title,args.target_weights,args.benchmark_name)
