...
pub fn load(catalog: &Path, target: &str) -> Result<Panel> {
    ...
    let mut dates=conn.prepare("SELECT trade_date::VARCHAR FROM (SELECT trade_date FROM observed_calendar WHERE is_observed_market_day AND trade_date<=?::DATE ORDER BY trade_date DESC LIMIT 1025) ORDER BY trade_date")?
    ...
    let begin = &dates[0];
    let mut codes=conn.prepare("SELECT DISTINCT ts_code FROM daily_aggregated WHERE trade_date BETWEEN ?::DATE AND ?::DATE AND open>0 AND high>0 AND low>0 AND close>0 AND volume_share>0 ORDER BY ts_code")?
    ...
    let mut stmt=conn.prepare("SELECT trade_date::VARCHAR,ts_code,open::DOUBLE,high::DOUBLE,low::DOUBLE,close::DOUBLE,volume_share::DOUBLE,amount_cny/nullif(volume_share,0)::DOUBLE FROM daily_aggregated WHERE trade_date BETWEEN ?::DATE AND ?::DATE AND open>0 AND high>0 AND low>0 AND close>0 AND volume_share>0 ORDER BY trade_date,ts_code")?;
