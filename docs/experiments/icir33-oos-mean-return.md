...
## 无 optimizer 基准：每日前 10% 等权、VWAP 到 VWAP

为区分 alpha 和 optimizer 的影响，另运行不使用 optimizer 的基准组合。每个信号日按 `alpha_daily` 取排名前 10%，以等权目标仓位在 T+1 VWAP 建仓，并在下一执行日的 VWAP 计算收益。结果在 [`top10pct-vwap-equal`](../../results/predict/icir33-lgbm-mean-h1-h5-v1/top10pct-vwap-equal)。

这是一条 position-to-position 回测：每期交易量由相邻目标权重的 L1 差计算。持续入选且目标权重不变的股票差额为零，不会被先卖再买。

| 指标 | 数值 |
| --- | ---: |
| 每期持仓数（最小 / 中位 / 最大） | 78 / 80 / 80 |
| 平均持仓保留率 | 19.0% |
| 平均单边换手 | 81.0% |
| 成本前总收益 | 56.9% |
| 10bp 成本后净总收益 | -81.3% |
| 累计交易成本 | 212.3% |
| 前 10% 的平均 1 日超额收益 | 1.34bp / 日 |
| 前 10% 的 1 日超额收益 t 值 | 1.04 |

无成本时，前 10% 的预测排序只有很弱、统计上不显著的优势；日频排名又大幅翻转，导致成本后完全不可用。因此本轮问题首先在 alpha 的稳定性和预测强度，而不是 optimizer。后续应做低频调仓、信号平滑/持有期约束和严格滚动的因子选择，再重新评估。
